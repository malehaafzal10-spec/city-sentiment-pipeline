"""
r04_extract_sentiment_historical.py — Script to fetch historical comments from MongoDB,
...
Usage in SLURM:
    python r04_extract_sentiment_historical.py --date 20260525
"""
...
def analyze_post(text, max_retries=4):
    """
    Sends the text to the local vLLM API and returns parsed JSON object.
    """
...
def main():
    log.info("=" * 60)
    log.info("PROCESS HISTORICAL MONGO COMMENTS WITH LOCAL VLLM API")
    
    # ── Handle pipeline arguments ───────────────────────────────────
    parser = argparse.ArgumentParser(description="Process Reddit comments deterministically by date.")
    parser.add_argument(
        "--date", 
        type=str, 
        required=True,
    ...
    if len(user_date) != 8 or not user_date.isdigit():
        log.error("Invalid date format. Must be exactly YYYYMMDD.")
        sys.exit(1)

    # Deterministically construct the exact run_id for both fetching and saving
    deterministic_run_id = f"run_{user_date}_local"

    log.info(f"Targeting Source & Destination Run ID: {deterministic_run_id}")
    log.info("=" * 60)
    ...
    # ── Fetch comments filtered by the deterministic Run ID ─────────────────
    query = {"run_id": deterministic_run_id}
    ...
    if TEST_MODE:
        ...
        try:
            ...
            posts = list(db_temp[SOURCE_COLLECTION].find(query).limit(TEST_LIMIT))
        except Exception as e:
            log.warning(f"MongoDB unavailable in TEST_MODE. Trying local fixture...")
    else:
        posts = list(db[SOURCE_COLLECTION].find(query))

    total_posts = len(posts)

    if total_posts == 0:
        log.warning(f"❌ No comments found in '{SOURCE_COLLECTION}' matching run_id: {deterministic_run_id}")
        return

    log.info(f"✅ Found {total_posts} comments exactly matching '{deterministic_run_id}'.")
    log.info("=" * 60)
    ...
    for idx, post in enumerate(posts):
        ...
        if not text:
            ...
            try:
                ...
                log.info("Stopping execution. Will save all currently processed comments...")
                break

        post['analysis'] = analysis_result
        total_processed += 1

        # ── Check relevance and attach TARGET RUN_ID ────────────────────────
        if analysis_result.get("relevant", "").lower() == "yes":
            post.pop('_id', None)
            
            # Re-assign the exact same run_id just to be safe
            post['run_id'] = deterministic_run_id
    ...
    log.info(f"Run ID Processed:        {deterministic_run_id}")
